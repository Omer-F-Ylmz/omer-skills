<#
  KURULUM-10 sync-off — sync-off-9.ps1 yontemi birebir, iki mod.

  Mod a (varsayilan): plugin-dev + claude-md-management CC'de acildi; bu iki plugin'in
           skill'leri synced kopyalarla ciftlendi. 8 cift ad kosulsuz off.
  Mod b (-Dist dist/yukle-10): claude.ai yuklemesi sonrasi kosulur.
           yeni/ altindaki zip'lerin hepsi off — CC karsiliklari var (cli-anything plugin,
           plugin-dev / claude-md-management plugin'leri, pixeljury CLI, headroom MCP).
           replace/ altindakiler CC'de baska aktif kopya yoksa acik kalir.

  Kural: yalniz synced'e ozgu `anthropic-skills:<ad>` anahtari yazilir. Bare-ad override
         native/yerel kopyayi da kapattigi icin YASAK (uretilirse throw).

  Kullanim:  .\tools\sync-off-10.ps1                      # -WhatIf: mod a sayimi
             .\tools\sync-off-10.ps1 -Apply               # mod a yazar
             .\tools\sync-off-10.ps1 -Dist dist\yukle-10  # -WhatIf: mod b sayimi
             .\tools\sync-off-10.ps1 -Dist dist\yukle-10 -Apply
  Idempotent: ikinci -Apply hicbir sey yazmaz. Yedek: settings.json.bak10s
#>
[CmdletBinding()]
param(
  [switch]$Apply,
...
# --- hedef kume: kural 1 = kosulsuz off, kural 2 = baska aktif kopya varsa off
$kural = @{}
if ($Dist) {
  $mod = "b (-Dist $Dist)"
  Get-ChildItem $Dist -Recurse -Filter *.zip | ForEach-Object {
    ...
    $z.Dispose()
    if ($_.FullName -like '*\yeni\*') { $kural[$ad] = 1 } else { $kural[$ad] = 2 }
  }
} else {
  $mod = 'a (plugin-dev + claude-md-management cift adlari)'
  'agent-development', 'command-development', 'hook-development', 'mcp-integration',
  'plugin-settings', 'plugin-structure', 'skill-development', 'memory-md-improver' |
    ForEach-Object { $kural[$_] = 1 }
}
...
# --- kural kalkani: bare-ad anahtar asla yazilmaz
...
if (-not $Apply) { "`n(-WhatIf) Yazmak icin ayni komuta -Apply ekle."; return }
if ($eklenecek.Count -eq 0) { "`ndegisiklik yok; dosyaya dokunulmadi."; return }

Copy-Item $ayar "$ayar.bak10s" -Force
...
$eskiAyar = Get-Content "$ayar.bak10s" -Raw | ConvertFrom-Json
function Ozet($x) {
